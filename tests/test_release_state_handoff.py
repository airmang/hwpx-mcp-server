...
def test_phase0_legacy_cap_precedes_every_core_5_resolution() -> None:
    ...
    matrix = names.index("Run public 5.6.0 compatibility install matrix")
    ...
    assert '"primaryApplication": "6.5.1"' in workflow
...
def test_automation_release_hands_off_without_global_promotion() -> None:
    ...
    assert "release-approved and currentPublic remains 5.5.0/6.5.1/1.5.0" in handoff_run
...
def test_identity_requires_complete_three_stack_remote_truth() -> None:
    ...
    if release["status"] != "released":
        assert release["currentPublic"]["plugin"] == "1.5.0"
    else:
