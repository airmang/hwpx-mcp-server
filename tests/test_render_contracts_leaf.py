...
def test_eager_import_succeeds_in_both_orders() -> None:
    ...
    _fresh_import(
        "import hwpx_automation.workflow.rendering as r; "
        "import hwpx_automation.workflow.render_queue as q; "
...
def test_leaf_module_has_no_package_dependencies() -> None:
    ...
    leaf = "hwpx_automation.workflow.render_contracts"
    assert graph[leaf] == set()


def test_compatibility_reexports_are_the_same_objects() -> None:
    from hwpx_automation.workflow import render_contracts, render_queue, rendering
...
def test_sdk_import_outside_allowlist_fails_the_ratchet(tmp_path) -> None:
    ...
    augmented["hwpx_automation.rogue"] = rogue
    ...
    assert "hwpx_automation.rogue" in importers
