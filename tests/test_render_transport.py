...
def tls_material(root: Path) -> dict[str, Path]:
    ...
    server_key, server_cert = issue("127.0.0.1", ExtendedKeyUsageOID.SERVER_AUTH, server=True)
    client_key, client_cert = issue("render-client", ExtendedKeyUsageOID.CLIENT_AUTH)
    paths = {name: root / f"{name}.pem" for name in ("ca", "server_cert", "server_key", "client_cert", "client_key")}
    paths["ca"].write_bytes(ca.public_bytes(serialization.Encoding.PEM))
    for path, value in ((paths["server_cert"], server_cert), (paths["client_cert"], client_cert)):
        path.write_bytes(value.public_bytes(serialization.Encoding.PEM))
    for path, value in ((paths["server_key"], server_key), (paths["client_key"], client_key)):
...
def test_transport_requires_https_except_explicit_loopback():
    ...
    with pytest.raises(ValueError, match="transport_auth"):
        RemoteRenderClientV2("https://example.com", secret=SECRET, transport_auth="none")  # type: ignore[arg-type]
...
def test_mtls_requires_ca_and_client_identity_on_both_ends(tmp_path):
    ...
    server = serve_private_queue(
        queue, secret=SECRET, host="127.0.0.1", port=0,
        certfile=tls["server_cert"], keyfile=tls["server_key"], client_ca_file=tls["ca"],
    )
    thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start()
    try:
...
def test_signed_https_is_explicit_and_preserves_hmac_auth(tmp_path):
    root = tmp_path / "queue"
    policy = RenderSecurityPolicy(sandbox_root=root / "sandboxes", transport_auth="signed_https")
...
def test_private_network_policy_rejects_wildcard_bind_unless_test_override(tmp_path):
    ...
    for host in ("0.0.0.0", "8.8.8.8", "::"):
        with pytest.raises(ValueError, match="private or loopback"):
            serve_private_queue(
                queue, secret=SECRET, host=host, port=0,
                certfile=tls["server_cert"], keyfile=tls["server_key"],
            )
    server = serve_private_queue(
        queue, secret=SECRET, host="0.0.0.0", port=0,
        certfile=tls["server_cert"], keyfile=tls["server_key"],
...
def test_server_render_client_wires_mtls_identity_from_environment(monkeypatch, tmp_path):
