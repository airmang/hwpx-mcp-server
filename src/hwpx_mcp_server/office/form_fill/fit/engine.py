...
class FitEngine:
        ...
        *,
        ...
        if measurement.fits:
            ...
            return self._apply_height_budget(
                result, value, slot, policy, wrap_lines, field_id
            )
    ...
    def _keep(
        ...
        if m.overflow:
            warnings.append("value exceeds the slot but mode=keep leaves it unchanged")
        return FitResult(
            ok=True,
            value=value,
            applied_value=value,
    ...
    def _expand_row(
        ...
        horizontal_risk = (
            self._longest_token_width(value, slot.font_pt) > slot.available_width
        )
        if horizontal_risk:
            warnings.append(
                ...
                best = FitResult(
                    ...
                    warnings=(
                        ...
                        if candidate == slot.font_pt
                            ...
                            f"font shrunk {slot.font_pt:g}pt → {candidate:g}pt to fit"
                        ]
                    ),
                )
                break
    ...
    def _height_verdict(
        self, slot: SlotMetrics, wrapped_lines: int, font_pt: float
    ) -> str:
    ...
    def _overflow(
        ...
        if action == "truncate":
            return self._truncate(
                value, slot, policy.with_(mode="truncate_with_report"), field_id
            )
    ...
    def _longest_prefix(value: str, slot: SlotMetrics) -> str:
        ...
        while low <= high:
            ...
            if (
                estimate_text_width(prefix, slot.font_pt) <= capacity * (1 - m.band)
                and m.fits
            ):
                best = mid
                low = mid + 1
