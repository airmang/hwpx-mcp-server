...
- 도구 등록, 입력·출력 스키마, health/capability 표면과 생성 문서는 하나의
  `ToolSpec` 레지스트리에서 만들어집니다. 4.2.1 공개 릴리스는 기본 121개, 고급 모드 포함
  132개 도구와 28개 skill-required 도구를 유지하며 계약 해시는
  `fff2c9093ca4677b`입니다. 최소 좌표는 core `3.3.0`, MCP `4.2.1`, skill
  `0.5.0`입니다.
- 4.0.0의 `f46ec677231b3a20`과 4.1.0의 `c127914cc3f4480e`는 역사적 계약입니다. 최소 버전 좌표도 canonical
  payload에 포함되므로 도구 이름·순서·스키마가 같더라도 4.2.1 해시와 섞어 쓰면
