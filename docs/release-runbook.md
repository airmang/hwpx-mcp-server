...
1. `python-hwpx 5.0.0` is observed on PyPI and as its GitHub Release;
2. canonical `python-hwpx-automation 6.0.0` and compatibility
   `hwpx-mcp-server 6.0.0` are observed on PyPI, and the automation GitHub
...
2. promote `currentPublic` to core 5.0.0, canonical automation 6.0.0, plugin
