...
>
> 릴리스 상태: `release-approved` — 6.0.1 train 발행이 승인되어 진행
...
이 문서는 아직 공개되지 않은 6.0.1 source candidate를 기준으로 합니다.
후보 좌표는 `python-hwpx 5.0.1 → python-hwpx-automation 6.0.1 →
...
6.0.1 후보의 기본 모드 119개(고급 모드 포함 총 127개)의 도구로 문서 생성,
