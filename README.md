...
현재 공개 릴리스는 `hwpx-mcp-server 4.0.0`이며 최소 호환 코어는 `python-hwpx 3.1.0`입니다.
`hwpx-plugin 0.3.0` 설치 번들은 재현성을 위해 MCP를 `==4.0.0`, 코어를 `==3.1.0`으로 고정합니다.
...
- 현재 4.0.0 릴리스의 기계 판독 계약 delta: [`docs/tool-contract-delta-4.0.0.json`](docs/tool-contract-delta-4.0.0.json)
