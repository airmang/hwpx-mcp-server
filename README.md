...
현재 공개 릴리스는 `hwpx-mcp-server 4.3.0`이며 최소 호환 코어는
`python-hwpx 3.3.1`, 대응 스킬 최소 버전은 `0.5.0`입니다. 기본 121개·고급 모드
포함 132개 도구와 28개 skill-required 도구는 유지되며 계약 해시는
`f82caecbcfc742e9`입니다.
...
- **🧩 다중 story 원자 편집** — `apply_document_commands` 한 호출에서 본문, 표 셀, 기존 구역의 단순 `BOTH` 머리글 텍스트를 함께 바꿀 수 있습니다. 하나라도 실패하면 출력 전체를 롤백하며, 구조를 안전하게 보존할 수 없는 rich/control 머리글은 변경 없이 거부합니다.
...
- 4.3.0의 생성 계약: [`docs/tool-contract.generated.json`](docs/tool-contract.generated.json)
- 4.3.0 승격 delta(classification-only 증명): [`docs/tool-contract-delta-4.3.0.json`](docs/tool-contract-delta-4.3.0.json)
- 역사적 4.0.0 기계 판독 계약 delta: [`docs/tool-contract-delta-4.0.0.json`](docs/tool-contract-delta-4.0.0.json)
