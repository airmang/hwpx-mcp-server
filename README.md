...
4. 필요하면 `save_as`로 별도 산출물을 남긴다.
5. 수정 후 다시 읽기 도구로 결과를 확인한다.
...
4. 결과물을 따로 넘겨야 하면 `save_as`로 분리 저장하고 검수한다.
...
| 복제 / 명시적 저장 | `copy_document`, `save`, `save_as` | 원본 보호, 별도 산출물 저장, handoff 직전 검수용 저장 경계다. |
...
이 카테고리의 도구는 대체로 대상 파일에 즉시 반영된다. 구조 변경 전에는 `copy_document`로 작업용 사본을 만들고, handoff 직전에는 `save_as`로 별도 산출물을 남기는 편이 안전하다.
...
이 카테고리는 수정 워크플로의 안전 장치다. 원본 보호와 handoff 시점을 명시적으로 만들 때 쓴다.

| 도구 | 설명 |
|---|---|
| `copy_document` | 원본을 건드리기 전에 작업용 사본 생성 |
| `save` / `save_as` | 명시적 저장, 별도 경로 저장, 검수 경계 만들기 |
...
변경 도구는 호출 시 즉시 저장됩니다. 검토용 사본이 필요하면 `copy_document`를 먼저 사용하고, 별도 납품본이 필요하면 `save_as`를 사용하세요.
